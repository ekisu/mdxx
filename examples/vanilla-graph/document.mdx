...
description: A self-contained MDX document with an interactive SVG, a Mermaid diagram, and highlighted code.
mdxx:
  format: 1
---
...
<style>
:root { color-scheme: light dark; font-family: system-ui, sans-serif; color: light-dark(#222, #e5e7eb); background: light-dark(white, #111827); }
body { margin: 0; }
...
pre { overflow-x: auto; padding: 1rem; border-radius: .35rem; }
...
Fenced source is highlighted during the build and remains readable without a browser highlighter:
...
The heading, prose, quotation, table, checklist, code, and Mermaid fences above are all Markdown. React is used directly only for the selectable graph and its detail readout. The presentation otherwise uses browser defaults, so the authored document remains easy to read and edit without specialized tooling.
