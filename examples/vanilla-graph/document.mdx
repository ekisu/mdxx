...
description: A self-contained MDX document with one interactive SVG.
mdxx:
  format: 1
---
...
This is an ordinary Markdown document with one interactive component. Everything needed to build it lives in this file.
...
The heading, prose, quotation, table, and checklist above are all Markdown. React is used only for the selectable graph and its detail readout. The presentation otherwise uses browser defaults, so the authored document remains easy to read and edit without specialized tooling.
