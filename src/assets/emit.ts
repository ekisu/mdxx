...
function outputName(path: string, digest: string): string {
  const extension = extname(path);
  const stem = basename(path, extension).replace(/[^A-Za-z0-9._-]+/g, "-") || "asset";
  return `${stem}.${digest.slice("sha256-".length, "sha256-".length + 8)}${extension.toLowerCase()}`;
}
...
export async function emitAssets(discovery: AssetDiscovery, outputDirectory: string): Promise<EmittedAssets> {
  ...
  for (const candidate of candidates) {
    ...
    if (!name) {
      name = outputName(candidate.path, candidate.digest);
